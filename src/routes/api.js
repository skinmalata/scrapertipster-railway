...
// Ticket Builder
const pathPredictions = path.join(__dirname, '../../predictions-cache.json');
router.get('/ticket-builder', (req, res) => {
  try {
    const targetOdds = parseFloat(req.query.target) || 20;
    const minOdds = parseFloat(req.query.minOdds) || 1.20;
    const maxOdds = parseFloat(req.query.maxOdds) || 3.00;
    const todayOnly = req.query.todayOnly !== '0';
    const maxTickets = parseInt(req.query.maxTickets) || 8;

    let predPath = pathPredictions;
    if (!fs.existsSync(predPath)) {
      predPath = path.join(__dirname, '../../public/data/predictions.json');
    }
    if (!fs.existsSync(predPath)) {
      return res.json({ success: false, tickets: [], message: 'No predictions data available' });
    }

    const data = JSON.parse(fs.readFileSync(predPath, 'utf8'));
    const today = data.date || new Date().toISOString().split('T')[0];

    // Collect all picks from all categories
    const picks = [];
    const seen = new Set();
    ...
    function addPicks(matches, category) {
      ...
      for (const m of matches) {
        ...
        const odds = parseFloat((100 / effectiveProb).toFixed(2));
        if (odds < minOdds || odds > maxOdds) continue;
        const key = `${matchName}|${m.tip}|${m.date}`;
    ...
    addPicks(data.cornersMatches, 'Corners');
    addPicks(data.cardsMatches, 'Cards');
    addPicks(data.teamToScore2PlusMatches, 'To Score 2+');
    addPicks(data.winstreakMatches, 'Win Streak');
    addPicks(data.losestreakMatches, 'Loss Streak');
    addPicks(data.drawstreakMatches, 'Draw Streak');

    if (picks.length === 0) {
      return res.json({ success: true, tickets: [], message: 'No picks match the criteria' });
    }
    ...
    // Score each pick by how well it divides into remaining target
    // This helps find combos that multiply to exactly target
    const scored = picks.map(p => ({
      ...p,
      score: Math.abs(Math.log(p.odds) - Math.log(targetOdds) / 4)
    }));
    ...
    function backtrack(start, current, product) {
      if (current.length >= 2) {
        const key = current.map(s => `${s.match}|${s.tip}`).sort().join('||');
      ...
      for (let i = start; i < pool.length; i++) {
        const newProduct = product * pool[i].odds;
        if (newProduct > targetOdds * 1.2) break;
        current.push(pool[i]);
        backtrack(i + 1, current, newProduct);
        current.pop();
      }
    }
    ...
    tickets.sort((a, b) => a.diff - b.diff);
    const top = tickets.slice(0, maxTickets);

    res.json({ success: true, tickets: top, totalCombos: tickets.length, targetOdds });
  } catch (e) {
