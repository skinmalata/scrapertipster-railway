...
router.get('/football-odds', async (req, res) => {
  res.json({ available: false, source: 'API-Football', message: 'Football odds are temporarily disabled', response: [] });
});

let liveTipsPending = null;

router.get('/live-tips', (req, res) => {
  res.json({ available: false, opportunities: [], message: 'Live tips are temporarily disabled.' });
});
...
router.get('/golden-tips', async function (req, res) {
  res.json({ available: false, opportunities: [], message: 'Golden tips are temporarily disabled.' });
});
