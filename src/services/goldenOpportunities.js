...
function checkDominantPressure(match) {
  ...
  var dominantName, dominantShots, dominatedShots, dominantCorners, domPossession;
  ...
  return {
    ...
    confidence: Math.round(confidence),
    reason: dominantName + ' dominating with ' + dominantShots + ' shots (' + dominatedShots + ' for opponent), ' + dominantCorners + ' corners' + (domPossession ? ', ' + domPossession + '% possession' : '') + ' at minute ' + elapsed + '.'
  };
}
...
function checkComebackMomentum(match) {
  ...
  var trailingName, leaderName, scoreDeficit, trailShotsTotal, leadShotsTotal;
  if (s.home < s.away) {
...
function checkSecondHalfPush(match) {
  ...
  var dominatorName, domShots, domPossession;
...
function pickBest(rules, match) {
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    if (tip && (!best || tip.confidence > best.confidence)) {
      best = tip;
    }
  }
  return best;
