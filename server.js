...
const CHROME_PATH = process.env.CHROME_PATH || process.env.PUPPETEER_EXECUTABLE_PATH || null;
...
async function scrapeDate(dateStr) {
  ...
  matchElements.each((i, el) => {
    ...
    if (homeTeam && awayTeam && bestProb >= 30) {
      matches.push({
        id: matchId++,
    ...
    if (homeTeam && awayTeam && over25 >= 30) {
      over25Matches.push({
        id: over25Id++,
        league: leagueInfo.league,
        country: leagueInfo.country,
...
async function fetchMatchResultsByDate(date) {
  ...
  try {
    ...
    console.log('Results fetch error:', error.message);
    return new Map();
  }
}
